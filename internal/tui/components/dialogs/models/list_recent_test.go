...
func TestModelList_RecentlyUsedSectionAndPrunesInvalid(t *testing.T) {
	...
	confPath := filepath.Join(cfgDir, "pluse", "pluse.json")
	...
	dataConfDir := filepath.Join(dataDir, "pluse")
	...
	origConfPath := filepath.Join(cfgDir, "pluse", "pluse.json")
	...
	dataConf := filepath.Join(dataDir, "pluse", "pluse.json")
