...
func TestPluseIgnore(t *testing.T) {
	tempDir := t.TempDir()
	t.Chdir(tempDir)
	...
	// Create a .pluseignore file that ignores .log files
	require.NoError(t, os.WriteFile(".pluseignore", []byte("*.log\n"), 0o644))
...
func TestShouldExcludeFile(t *testing.T) {
	...
	// Create .pluseignore file
	pluseignoreContent := "custom_ignored/\n"
	if err := os.WriteFile(filepath.Join(tempDir, ".pluseignore"), []byte(pluseignoreContent), 0o644); err != nil {
		t.Fatalf("Failed to create .pluseignore: %v", err)
	}
	...
	require.True(t, ShouldExcludeFile(tempDir, customIgnored), "Expected custom_ignored to be ignored by .pluseignore")
...
func TestShouldExcludeFileHierarchical(t *testing.T) {
	...
	// Create .pluseignore in subdir that ignores normal_nested
	subPluseignore := "normal_nested/\n"
	if err := os.WriteFile(filepath.Join(subDir, ".pluseignore"), []byte(subPluseignore), 0o644); err != nil {
		t.Fatalf("Failed to create subdir .pluseignore: %v", err)
	}

	// Test hierarchical ignore behavior - this should work because the .pluseignore is in the parent directory
	require.True(t, ShouldExcludeFile(tempDir, nestedNormal), "Expected normal_nested to be ignored by subdir .pluseignore")
